...
func (s *Service) CastResistance(ctx context.Context, battleID primitive.ObjectID, casterUsername string, casterUserID string) error {
	// Get battle
	var battle Battle
	...
	if err == nil {
		return errors.New("Resistance spell is already active in this battle")
	}
	...
	// Double defense for all warriors
	updatedCount := 0
	for _, warrior := range warriors {
		newDefense := warrior.Defense * 2
		...
		if err != nil {
			log.Printf("Failed to update warrior %s defense: %v", warrior.Name, err)
			continue
		}
	...
	// Log to Redis
	go func() {
		message := fmt.Sprintf("🛡️ SPELL CAST: Resistance! All warrior defense doubled! (%d warriors affected)", updatedCount)
