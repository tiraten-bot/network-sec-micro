	...
	return func(c *fiber.Ctx) error {
		...
		for idx, cr := range croutes {
			...
			if cr.cfg.PathPrefix != "" {
				if !hasPrefix(path, cr.cfg.PathPrefix) {
					continue
				}
			}
			...
			// method policy
			if len(cr.denySet) > 0 {
				if _, deny := cr.denySet[method]; deny {
					return c.SendStatus(fiber.StatusMethodNotAllowed)
				}
			}
			if len(cr.allowSet) > 0 {
				if _, allow := cr.allowSet[method]; !allow {
					return c.SendStatus(fiber.StatusMethodNotAllowed)
				}
			}
			...
			if cr.cfg.RateLimit != nil && cr.cfg.RateLimit.Enabled {
				...
				if cr.cfg.RateLimit.UseRedis && rdb != nil {
					...
					if err != nil {
						return c.Status(fiber.StatusInternalServerError).SendString("rl err")
					}
					if !allowed {
			...
			if cr.cfg.Quota != nil && cr.cfg.Quota.Enabled && rdb != nil {
				qkey := c.IP()
				if kh := cr.cfg.Quota.KeyHeader; kh != "" {
					if v := c.Get(kh); v != "" {
						qkey = v
					}
				}
				if cr.cfg.Quota.Hourly > 0 {
					...
					if err != nil {
						return c.Status(fiber.StatusInternalServerError).SendString("quota err")
					}
					if !ok {
						return c.Status(fiber.StatusTooManyRequests).SendString("hourly quota exceeded")
					}
				}
				if cr.cfg.Quota.Daily > 0 {
					...
					if !ok {
						return c.Status(fiber.StatusTooManyRequests).SendString("daily quota exceeded")
					}
				}
			}

			// transforms
			for k, v := range cr.cfg.HeadersSet {
				c.Request().Header.Set(k, v)
			}
			for _, k := range cr.cfg.HeadersRemove {
				c.Request().Header.Del(k)
			}
			if len(cr.cfg.QueryInject) > 0 {
				q := c.Request().URI().QueryArgs()
				for k, v := range cr.cfg.QueryInject {
					q.Set(k, v)
				}
			}
			...
			if len(cr.cfg.Upstreams) > 0 {
				...
				if cr.cfg.LoadBalancing == "least_connections" {
					...
					for _, up := range cr.cfg.Upstreams {
						if until, ok := st.ejectedUntil[up]; ok && time.Now().Before(until) {
							continue
						}
						cur := st.active[up]
						if cur < min {
					...
					if pick == "" {
						pick = cr.cfg.Upstreams[st.lbIdx%len(cr.cfg.Upstreams)]
						st.lbIdx++
					}
					st.lbMu.Unlock()
				} else {
					...
					for i := 0; i < len(cr.cfg.Upstreams); i++ {
						...
						if until, ok := st.ejectedUntil[candidate]; ok && time.Now().Before(until) {
							continue
						}
						pick = candidate
						st.lbIdx = (st.lbIdx + i + 1) % len(cr.cfg.Upstreams)
						break
					}
					if pick == "" {
						pick = cr.cfg.Upstreams[st.lbIdx%len(cr.cfg.Upstreams)]
			...
			if cr.cfg.CanaryUp != "" && cr.cfg.CanaryPct != "" {
				...
				if len(rid) > 0 {
					last := rid[len(rid)-1]
					pct := 10
					if p, err := parsePercent(cr.cfg.CanaryPct); err == nil {
						pct = p
					}
					if int(last)%100 < pct {
						base = cr.cfg.CanaryUp
					}
				}
			}
			...
			if len(cr.cfg.Aggregates) > 0 {
				...
				if cr.cfg.Cache != nil && cr.cfg.Cache.Enabled && c.Method() == fiber.MethodGet {
					...
					if len(cr.cfg.Cache.VaryHeaders) > 0 {
						for _, h := range cr.cfg.Cache.VaryHeaders {
							key += "|" + c.Get(h)
						}
					}
...
func executeAggregate(c *fiber.Ctx, cr compiledRoute) (map[string]interface{}, int, map[string]string, error) {
	...
	if policy != nil && policy.TimeoutS > 0 {
		overallTimeout = time.Duration(policy.TimeoutS) * time.Second
	}
	...
	for _, call := range cr.cfg.Aggregates {
		...
		go func() {
			...
			for hk, hv := range call.Headers {
				req.Header.Set(hk, hv)
			}
			httpClient := &http.Client{Timeout: time.Duration(call.TimeoutS) * time.Second}
			if call.TimeoutS == 0 {
				httpClient.Timeout = overallTimeout
			}
			resp, err := httpClient.Do(req)
			if err != nil {
				resCh <- result{name: call.Name, err: err}
				return
			}
	...
	for i := 0; i < len(cr.cfg.Aggregates); i++ {
		...
		case r := <-resCh:
			if r.err != nil {
				if policy != nil && policy.Partial != "ignore" {
					return nil, fiber.StatusBadGateway, nil, r.err
				}
				continue
			}
		...
		case <-ctx.Done():
			if policy != nil && policy.Partial != "ignore" {
				return nil, fiber.StatusGatewayTimeout, nil, ctx.Err()
			}
		}
	}
	...
	if successes == 0 {
		return merged, fiber.StatusBadGateway, headers, nil
	}
